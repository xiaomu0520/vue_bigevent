...
// 原因:
// 默认打包，index.htrml引入其他打包的文件使用的是：绝对地址
// 地址是以/开头，（要找到当前index.html打开时所在的服务器根地址（根文件夹））
// 就得确保你的vscode+liserver插件打开时，vscode根目录得直接是dist文件夹
// 如果把dist文件夹交给后台运维工程师部署到服务器上，dist下内容就得在服务器根目录才行
// 但是服务器一般会有多个项目,不让你用根目录直接放一个项目
...
// 服务器的根目录文件夹
// 生产环境准备上线了，这个时候就得用相对地址publicPath：'./'
// 路径不以/开头其实就是./开头（默认的）
